...
# write_rds(fin_reg_df, paste0(file.path(Sys.getenv("USERPROFILE")),
#                             "\\Documents\\SandBox",
#                            "\\FinSynchFinReg.rds"))
...
# write_rds(country_df, paste0(file.path(Sys.getenv("USERPROFILE")),
#                             "\\Documents\\SandBox",
...
This work analyzes the relationship between financial integration and financial cycles synchronization. Financial integration refers to the linkage of different financial markets while financial cycles synchronization refers to the co-movement of the main components of the financial cycle (credit, house and shares prices). I identify the 
...
The estimation results can be subject to an endogeneity problem for reasons of omitting relevant variables or reverse causality. In order to address this issue, I take several steps: first, I augment the specified regression with a set of variables to control for other economic factors that can affect both financial cycle synchronization and banking integration. I use the index of @Chinn2010 to control for foreign exchange stability, monetary independence, and financial openness. I also include trading linkages to control for cross country trading activity\footnote{All the variables are explained in the Appendix.}. Second, bank linkages enter in my specification in a lagged form, which should attenuate the reverse causality problem.
...
periods or in highly developed financial markets integration is associated with higher synchronization relative to baseline case (the overall effect remains negative). That finding is in line with the theory that suggests that the "risk propagator" feature is present in these settings and thus the synchronization should increase relative to base setting. Banking (and financial) crises are often contagious events that are being hard to contain in locally and thus produce common shocks that result in higher synchronization. 
...
One implication for the Euro block (that is characterized by one currency and very developed financial markets) is that the sum of the coefficients suggest that during a financial crises European Union's banking system may have a cumulative effect of shock propagation and not
risk absorbency.
