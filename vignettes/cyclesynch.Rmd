...
This work studies the relationship between financial cycles synchronization and financial integration. A recent line of research studies the relationship between business cycles synchronization and financial integration. For example, @Kalemli-Ozcan2013JF find that output cycle synchronization is negatively affected by financial integration. The authors explain the connection as follows: if a country is hit with a negative productivity shock global banks will reduce credit to that country and increase credit to another country thus exacerbating negative output effect in the former country and facilitating positive output effect in the later. In this paper, I test whether the same mechanism applies to financial cycles.
...
To this end, I follow the methodology suggested by @Kalemli-Ozcan2013JF. Specifically, I use their measures of cycles synchronization and banking linkages and run a panel regression controlling for common and time-invariant unobservable factors. I also control for some other economic factors that can affect both financial cycle synchronization and banking integration. While the authors focus on 18 (High-Income) OECD countries I expand my sample to all OECD countries.
...
@Kalemli-Ozcan2013JF focused on 18 (High-Income) members of OECD for 1978-2006 years.
I expand the sample to include all the OECD countries for `r paste(range(fin_reg_df_annual$Date), collapse = "-")` period. In order to address the differences between OECD countries, I split the country-pair data to 3 different groups: High-Income countries that correspond to the authors' sample, Low-Income countries that are missing from their sample and Cross (High-Low Income) group where one country belongs to High-Income and the other to Low-Income group.
...
To account for the fact that the effect of financial integration depends on the level of domestic financial development I include an index of financial development and an interaction term between financial development and banking integration. Data on financial development index comes from @Sviry2016 and includes an index of financial development of all the countries in ma sample for `r paste(range(fin_reg_df_annual$Date[!is.na(fin_reg_df_annual$FD_tot)]), collapse = "-")` period.
...
I divide the sample into three country-pairs subgroups: 
\begin{itemize}
  \item
  ...
  \item
    ...
    Lithuania, Luxembourg, Mexico, New Zealand, Norway, Poland, Slovak
    Republic, Slovenia, and Turkey. Low-Income country-pairs are pairs where each
    country belongs to Low-Income
    group.
  \item
...
Summary statistics of the data suggest that High-Income countries are more financially integrated and developed relative to Low-Income countries. Banking crises seem to be less frequent in High-Income countries. There seems to be no substantial difference in the average financial synchronization among the groups.
...
Data on financial development is available only for `r paste(range(fin_reg_df_annual$Date[!is.na(fin_reg_df_annual$FD_tot)]), collapse = "-")` period. That means that the inclusion of the data comes at the cost of reducing sample size. For this reason, I run several regressions, incrementally adding the variables to the estimation equation. 
...
Table \ref{fd} shows the results of the regression with the financial development index included. Banking linkages now enter the equation with a negative sign, although the results are still insignificant (except for Low-Income group). The interaction between banking linkages and financial development show that banking integration in highly developed financial markets is associated with more synchronized cycles. That finding offers some support for the hypothesis that in advanced countries a high level of financial development enables the use of sophisticated financial instruments that can transmit common shocks. The results in Low-Income group are (again) in the opposite direction relative to other groups. Bank linkages are associated with more synchronized cycles while financial development is association is negative. This may be due to some structural differences between Low-Income and High-Income countries but on the other hand, a relatively small number of observations can results in overfitting as reflected by the relatively high value of $R^{2}$ measure.
...
Table \ref{crisis_fd} presents estimation results that include crises and financial development data. Once crises and financial development contributions are controlled for, the association of bank linkages (that is the effect of bank linkages in normal times in countries with "basic" level of financial development) is negative, namely that bank linkages are associated with less synchronized cycles. This result is statistically significant in High-Income and All groups. The crises effect continues to be significantly positive across all groups and the financial development effect is significantly positive among High-Income, Cross and All groups and negative in the Low-Income group.
...
Table \ref{baseline_2007} present the baseline estimation results. When financial development effect is uncontrolled for the results suggest a positive association between banking integration and financial cycles synchronization (significant in all but Low-Income group). However, once financial development is accounted for (results are shown in table \ref{fd_2007}), we can see that the positive effect comes from financial development. The coefficients of banking linkages turn negative (although insignificant) meaning that banking linkages are associated with less synchronized financial cycles. This finding offers some support to the hypothesis about banking flows that serve as rebalancing mechanism. 


\subsection{Alternative empirical specifications}

In this section, I exclude the country-pair specific linear trend in order to test the robustness of my estimation. Table \ref{crisis_fd_no_trens} presents the results. The only result that remains statistically significant is the crisis effect - during banking crises banking integration is associated with more synchronized cycles.
