...
hi_income_string = paste(paste(countries_list$strong_countries[
  1:length(countries_list$strong_countries) - 1],collapse = ","), "and",countries_list$strong_countries[length(countries_list$strong_countries)])
...
cont_df_fd = fin_reg_df_annual %>% 
  select(c("Date", "CountryPair","Fin_synch","bank_gdp",
           control_vars,"Time_trend","FD_tot")) %>% 
  filter(complete.cases(.))
...
```{r estimation_panel_reg_2007, eval=FALSE}

cont_df_2007 = fin_reg_df_annual %>% 
    select(c("Date", "CountryPair","Fin_synch","bank_gdp",
             "Time_trend", control_vars)) %>% 
  filter(complete.cases(.)) %>% 
  filter(Date <= 2006)
...
```{r estimation_panel_reg_fd_no_trend}

cont_df_fd_no_trend = fin_reg_df_annual %>% 
  select(c("Date", "CountryPair","Fin_synch","bank_gdp",
           control_vars,"Time_trend","FD_tot")) %>% 
  filter(complete.cases(.))
...
reg_list$panel_fd_no_trend = make_strata_reg_list(
  countries_list = countries_list,
  reg_df = cont_df_fd_no_trend,
  reg_formula = formula(cont_fd_formula))



rm(cont_df_fd_no_trend,cont_fd_formula)


```


```{r estimation_panel_reg_fd_2007_no_trend, eval=FALSE}

cont_df_fd_2007_quasi_cor = fin_reg_df_annual %>% 
    select(c("Date", "CountryPair","Fin_synch_quasi_cor","bank_gdp",
             control_vars,"Time_trend","FD_tot")) %>% 
  filter(complete.cases(.)) %>% 
  filter(Date <= 2006)

cont_fd_formula_2007 = paste("Fin_synch_quasi_cor~ lag(bank_gdp,1) * Crises",
                         "lag(bank_gdp,1) *FD_tot",
                         paste(control_vars, collapse = "+"),
                        "CountryPair:Time_trend",
                     sep = "+")
...
reg_list$panel_fd_2007 = make_strata_reg_list(
  countries_list = countries_list,
  reg_df = cont_df_fd_2007_quasi_cor,
  reg_formula = formula(cont_fd_formula_2007))



rm(cont_df_fd_2007_quasi_cor,cont_fd_formula_2007)
...
Hi Income group includes Australia, Austria, Belgium, Canada, Switzerland, Germany, Denmark, Spain, Finland, France, United Kingdom, Ireland, Italy,
Japan, Netherlands, Portugal, Sweden and United States. The Low Income group includes Chile, Czech Republic, Estonia, Greece, Hungary, Iceland, Israel,
Korea, Latvia, Lithuania, Luxembourg, Mexico, New Zealand, Norway and Poland,
Slovak Republic ,Slovenia ,Turkey. The plot shows that each group is characterized by it own trend : while Hi Income countries exhibit upward trend, Low Income countries show dramatic drop (probably as a result of GFC). The Cross groups sample (being the "average" of the two previous groups) doesn't show as significant trend as the other groups. In the regression analysis I use country-pair specific trend to account for this non stationary behaviour. 
...
temp_df = fin_reg_df_annual %>% 
  select(Fin_synch, bank_gdp, FD_tot, Crises, CountryPair_Category) %>% 
  mutate(CountryPair_Category = recode(
    CountryPair_Category,
...
[@Bhadir2015] show that financial development convergence takes places. Countires with high level of financial development exhibit lower credit growth rates in comparison to countries with low level of financial development.

[@Kil2017] confirm the financial development convergence for EU15 countries. They find that both credit and stock markets exhibit convergence meaning that growth rates decrease at high levels. 
...
After adressing all the previously described effect my full specification is 
...
Unfortunately data on financial development is available only for `r paste(range(fin_reg_df_annual$Date[!is.na(fin_reg_df_annual$FD_tot)]), collapse = "-")` period. That means that inclusion of the data comes at a cost of reducing sample size. For this reason I run two regressions, incrementaly adding the variables to the estimation equation. This process allows me to test the robustness of the results.
...
```{r output_panel_reg_fd_2007, results="asis", eval=FALSE}
...
other economic factors that can bias the identification and lagged banking linkages to deal with reverse causality. To the extent that these measures adequately address the endogeineity issue causal interpretaion can be assigned.
...
```{r output_panel_reg_fd_no_trend, results="asis"}
