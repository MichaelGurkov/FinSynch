...
formula_vars = control_vars[!grepl(pattern = "FD_tot",control_vars)]

cont_formula = paste("Fin_synch~ bank_gdp * Crises",
                     "bank_gdp * FD_tot",
                     "lag(Fin_synch, 1)",
                     paste(control_vars[!grepl(pattern = "Time_trend",
                                               control_vars)],
                           collapse = "+"),sep = "+")

cont_formula_lag = paste(cont_formula,"|. -bank_gdp + lag(bank_gdp, 1)")

cont_formula_delta = paste("Fin_synch_delta~ bank_gdp_delta * Crises",
                     "bank_gdp_delta * FD_tot",
                     "lag(Fin_synch_delta, 1)",
                     paste(control_vars[!grepl(pattern = "Time_trend",
                                               control_vars)],
                           collapse = "+"),sep = "+")
...
table_names = c("bank_gdp","Crises",
                ...
                "bank_gdp:FD_tot",
                "lag(Fin_synch, 1)",
                "trade_gdp","FX_stab_tot","MI_ind_tot",
                "FO_ind_tot","WGI_tot")
...
raw_data$bis_lbs = import.bis.lbs.data(countries_vec = countries_list$oecd_countries) %>% 
  mutate(Date = as.yearqtr(Date, format = "%Y-Q%q"))
...
#   mutate(bank_gdp_detrended = residuals(lm(bank_gdp ~ Time_trend))) %>% 
#   mutate(Fin_synch_detrended = residuals(lm(Fin_synch ~ Time_trend))) %>% 
#   mutate(Harmon_both_detrended = residuals(lm(Harmon_both_Index ~ Time_trend))) %>% 
#   ungroup()
...
```{r lag_estimation_bank_gdp, eval=FALSE}
...
```{r lag_estimation_bank_gdp_controls}
