...
plot(net, vertex.size = sqrt(degree(net)),
     edge.arrow.size = 0)
...
\section{Hypothesys formulation and estimation results}
...
$FinSynch_{i,j,i} = \alpha_{i,j} + \delta_{t} + \beta BankLinkages_{i,j,t} + \gamma_{1} Crises + \gamma_{2} BankLinkages_{i,j,t} * Crises + \theta_{1} FD + \theta_{2} BankLinkages_{i,j,t} * FD  + Controls_{i,j,t} + \varepsilon_{i,j,t}$
...
Estimation results are shown in table \ref{panel_reg_controls}. Banking linkages are associated with more synchronized cycles in total sample (column 1). Breaking the sample to Hi and Low income countries reveals that there is no significant association in the Hi income countries sample. Financial cycles are more synchronized during banking crisis and banking integration's "shock propagating" feature is amplified during these periods. Financial development levels are inversely associated with synchronization but there is no significant interaction between banking integration and financial development effect

\section{Robustness}

I ran a regression with lagged banking linkages as an instrumental variable. Arguably 

```{r output_panel_reg_lag, results="asis", eval=output_status}
...
```{r output_panel_reg_delta, results="asis", eval=output_status}
