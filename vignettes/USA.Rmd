...
```{r plot_usa_bank_linkages,fig.cap="Israel's cross border credit\\label{plot_usa_bank_linkages}", fig.pos="h", out.extra=""}

net_df = raw_data$bis_lbs %>%
  filter(grepl("United_States",CountryPair)) %>%
  ...
  ungroup() %>%
  mutate(CountryPair = str_remove_all(CountryPair,"United_States")) %>%
...
ggplot(net_df, aes(x = reorder(Country, Balance), y = Balance)) +
  ...
  labs(x = "", y = "Total cross border bilateral credit (USD billions)",
       title = paste("USA's total cross border bilateral credit",
