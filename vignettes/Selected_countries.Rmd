...
plot_cycles_cap = paste0(
  "This figure plots the financial cycles of US, UK and Israel",
  " The financial cycle is defined as the simple average of log returns",
  " of the three cycle components (total credit, house prices",
  " and share prices).",
  "\\label{plot_cycles}")

plot_correl_cap = paste0(
 "The figure plots the within-country pair and within-year correlation between",
 " financial synchronization. (Fin Synch) in the vertical axis",
" and banking integration (Bank Linkages) in the horizontal axis.",
...
" effects and year fixed effects. Then I plot the residuals",
" of the synchronization regression in the vertical axis",
" against the residuals from the banking integration regression",
...
plot_bank_linkages_cap = paste0(
  "The cross border credit (millions USD) for top 5 counter parties of two",
  " big financial centers (United States and United Kingdom), and small",
...
for two big financial centers (United States and United Kingdom), as well as small open market country (Israel). The distribution of the bilateral cross border credit is shown in Figure \ref{plot_bank_linkages}. United States and United Kingdom are the largest counterparty to each other, Unites States is also the largest cross border credit counterparty for Israel.
...
```{r plot_cycles, fig.cap=plot_cycles_cap, fig.pos="h", out.extra=""}
...
ggplot(country_df %>% 
  ...
  labs(x = "", y = "Rate of change (YoY)",
       title = paste0("Financial cycles of US, UK and Israel")) + 
  theme_bw() + 
...
The figure plots the residuals of this first step regressions, the trend line shows the (slightly) negative association between financial synchronization and banking integration in the all country pair.
