...
```{r libraries_selected_countries}
...
net_df = map(
  ...
  temp_df = raw_data$bis_lbs %>%
    filter(grepl(temp_country,country_pair)) %>%
    filter(date == as.yearqtr("2018 Q4")) %>% 
    select(country_pair, balance) %>%
    group_by(country_pair) %>%
    summarise(balance = mean(balance, na.rm = TRUE) * 10 ^ (-3),
              .groups = "drop") %>%
    mutate(country_pair = str_remove_all(country_pair,temp_country)) %>%
    mutate(country_pair = str_remove_all(country_pair,"-")) %>%
    mutate(country_pair = str_replace_all(country_pair,"_"," ")) %>%
    rename(country = country_pair) %>% 
...
bank_clean = plm(sub("fin_synch","bank_gdp",
                     gsub("lag(bank_gdp,1) *","",
                          ...
                          fixed = TRUE),
                     fixed = TRUE),
                 data = country_pair_df, index = c("country_pair","year"),
                 model = "within", effect = "twoways")

synch_clean = plm(gsub("lag(bank_gdp,1) *","",
                       reg_formula_list$full_spec,
                       fixed = TRUE),
                  data = country_pair_df, index = c("country_pair","year"),
                  model = "within", effect = "twoways")

correl_df = list(bank_clean$model$country_pair,
...
ggplot(cycles_df %>% 
  filter(country %in% c("Israel","United_States",
                        "United_Kingdom")) %>% 
  filter(year >= 1995),
  aes(x = year, y = fin_cycle * 100, color = country,
      group = country)) + 
  geom_line() + 
