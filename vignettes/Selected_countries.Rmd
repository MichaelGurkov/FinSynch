...
```{r plot_bank_linkages,fig.cap="Israel's cross border credit\\label{plot_bank_linkages}", fig.pos="h", out.extra=""}

plot_list = map(unique(net_df$Source),function(temp_name){
  
  net_df %>% 
    filter(Source == temp_name) %>% 
    slice_max(Balance,n = 10) %>% 
    ggplot(aes(x = reorder(Country, Balance), y = Balance)) + 
    geom_col(width = 0.5) +
    labs(title = temp_name) + 
    xlab(NULL) + 
    ylab(NULL) + 
...
ggplot(country_df %>% 
  ...
  filter(Country %in% c("Israel","United_States","United_Kingdom")) %>% 
  filter(Date >= 1995),
