...
roll_cor_df = cycles_df %>% 
  ...
  select(country_pair, year, fin_cycle,fin_cycle_counter) %>% 
  group_by(country_pair) %>% 
  arrange(year) %>% 
  mutate(fin_roll_cor = slide2_dbl(fin_cycle, fin_cycle_counter, cor,
                                .before = roll_cor_len,.complete = TRUE)) %>% 
  ungroup() %>% 
  select(country_pair, year, fin_roll_cor) %>% 
  filter(complete.cases(.))



reg_list$roll_cor_reg = plm(formula = str_replace(reg_formula_list$full_spec,
                                                  "fin_synch","fin_roll_cor"),
                            data = country_pair_df %>% 
                              left_join(roll_cor_df,
                                        by = c("country_pair", "year")) %>% 
                              select(-fin_synch),
                            model = "within",effect = "twoways",
                            index = c("country_pair","year"))
...
all_star_reg = all_star_reg %>% 
  sub(pattern = "\\textit{Note:}  & \\multicolumn{3}{l}{\\parbox[t]{8cm}{",
      ...
      fixed = TRUE) %>% 
  sub(pattern = "\\multicolumn{2}{c}{GFC} & Roll",
...
reg_list$no_trend = plm(formula = str_remove_all(reg_formula_list$baseline,
