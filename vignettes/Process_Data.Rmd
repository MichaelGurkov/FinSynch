...
euro_memebership_df = calculate_euro_membership(raw_data)
...
country_pair_df = list(fin_synch_df, bank_link, trade_link,
  ...
  mutate(across(c("crisis_ind", "fo_ind", "fx_stab","fd_ind"),
                ~replace_na(.,0)))
...
country_pair_df = country_pair_df %>% 
  left_join(euro_memebership_df, by = "country_pair") %>% 
  mutate(euro_member = as_factor(euro_member))

# rm(list = ls()[!ls()%in% c("country_pair_df","raw_data")])

```

```{r filter_country_pair, eval=FALSE}
