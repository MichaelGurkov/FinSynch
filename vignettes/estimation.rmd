...
reg_formula_list = list(
  ...
  full_spec = paste(
    "fin_synch_diff ~ lag(bank_gdp,1) * crisis_ind * euro_member",
    paste(control_vars,
          collapse = "+"),
    sep = "+"
  )
...
reg_formula_list = c(reg_formula_list, cor_formula_list)
...
reg_list = map(reg_formula_list, function(temp_list){
  ...
  #   inner_join(temp_list$index, by = "country_pair")
...
extract_coeffs = function(temp_reg_obj, temp_name){
  ...
  temp_se = sqrt(diag(vcovHC(temp_reg_obj,cluster = "group")))
