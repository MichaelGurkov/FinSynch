...
extract_coeffs = function(temp_reg_obj, temp_name){
  ...
  temp_significance = summary(temp_reg_obj)$coefficients[,4]
  
  temp_sign = sign(temp_coefs)
  
  temp_ind = names(temp_coefs) %in% c("lag(bank_gdp, 1)",
  ...
  temp_df = tibble(var_name = names(temp_sign)[temp_ind],
                   ...
                   model = temp_name) %>% 
    mutate(significance = if_else(significance < 0.1,
...
temp_coef %>% 
  ...
  extract(model, into = c("model","sample"), "(.*)_([a-z]+_countries$)") %>% 
  mutate(sample = str_remove(sample, "_countries")) %>%  
  # mutate(model = factor(model,
  #                       levels = c("full_spec","crises","baseline"))) %>% 
  filter(complete.cases(.)) %>% 
  ggplot(aes(x = var_name, y = model, 
             color = as_factor(value), shape = as_factor(significance))) + 
  geom_point() + 
  facet_wrap(~sample) + 
  xlab(NULL) + ylab(NULL)
