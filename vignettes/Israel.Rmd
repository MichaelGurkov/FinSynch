...
```{r plot_israel_vars, fig.height=7, fig.height=7}

ggplot(country_df %>%
         filter(Country == "Israel") %>%
         select(Date, Total_Credit_real, HousePrice_real,
                SharePrice_real,GDP_real, FD) %>%
         rename_all(.funs = list(~gsub(pattern = "_real",
                                       replacement = "",
                                       x = .,
                                       fixed = TRUE))) %>%
         mutate(GDP = GDP * 10 ^ (-3)) %>%
